...
from archs.voice_synth.mellotron.text import text_to_sequence, get_arpabet, cmudict
import ipdb

import logging
logging.basicConfig(filename='events2eventsarpabet.log',level=logging.ERROR)

CMUDICT_PATH = "archs/voice_synth/mellotron/data/cmu_dictionary"
...
def musicxml2score(filepath, bpm=60):
    ...
    for i in range(len(data.parts)):
        ...
        events = []
        for k in range(len(part.getElementsByClass(['Note']))):
            event = part.getElementsByClass(['Note'])[k]
            if not event.lyric:
                continue

            if isinstance(event, m21.note.Note):
                freq = event.pitch.frequency
                token = event.lyrics[0].text if len(event.lyrics) > 0 else SILENT_PHO
...
def get_data_from_musicxml(filepath, bpm, phoneme_durations=None,
                           convert_stress=False, method1=False):
    ...
    for k, v in score.items():
        ...
        def phoneme_track_to_eventsarapabet(v):
            PHONEME = 0
            FREQ = 1
            START_TIME = 2
            END_TIME = 3
            ...
            while i < len(v):
                ...
                if i < (len(v) - 2):
                    next_syll_onset = v[i + 1][START_TIME]
                else:
                    next_syll_onset = -1

                if syll[START_TIME] == last_onset:
                    i += 1
                    continue

                if syll[PHONEME][0] != '{' and start == False:
                    i += 1
                    start = False
                    combination = []
                    continue
                ...
                # print(syll, combination)
                # ipdb.set_trace(context=5)

                for pho in syll[PHONEME].split():
                    
                    if current_word != i and len(combination) > 0:
                        #If new syll is not consecutive to the last syll
                        if (abs(syll[START_TIME] - combination[-1][END_TIME]) >= 0.001):
                            comb = [SILENT_PHO_BETWEEN]; comb.extend([ combination[-1][FREQ], combination[-1][END_TIME], syll[START_TIME] ]) #combination[-1][FREQ]
                            combination.append(comb)

                            # #Close the word
                            # combination[-1][PHONEME] += '}'
                            # #Append to the events container                            
                            # events.append(combination)

                            # #Add SILENT_PHO between

                    if (syll[END_TIME] > next_syll_onset) and (next_syll_onset != -1):
                        new_syll = syll
                        new_syll[END_TIME] = next_syll_onset
                        comb = [pho]; comb.extend(new_syll[1:])
                    else:
                        comb = [pho]; comb.extend(syll[1:])

                    combination.append(comb)

                    current_word = i

                if start and syll[PHONEME][-1] == '}':
                    start = False

                    #Add SILENT_PHO if silence in the beginning
                    if (len(events) == 0 and combination[0][START_TIME] != 0):
                        events.append([ [SILENT_PHO, 0, 0, combination[0][START_TIME]] ])

                    elif last_end_time != -1:
                        #If there is gap between words, fill with SILENT_PHO
                        if last_end_time != combination[0][START_TIME]:
                            events.append([ [SILENT_PHO, 0, last_end_time, combination[0][START_TIME]] ])

                    events.append(combination)
                    last_end_time = combination[-1][END_TIME]
                
                last_onset = syll[START_TIME]
        ...
        if method1:
            ...
            events_arpabet = phoneme_track_to_eventsarapabet(v)
