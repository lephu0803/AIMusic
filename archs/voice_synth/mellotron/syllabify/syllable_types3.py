...
class Syllable(object):
	...
	def phonemes(self):
		phons = ''
		phons += str(self.get_onset()) if self.has_onset() else ''
		phons += str(self.get_nucleus()) if self.has_nucleus() else ''

		if self.has_coda():
			phons += str(self.get_coda())[:-1]
		else:
			phons = phons[:-1]
...
class Vowel(object):
	...
	# Representation
	def __str__(self):
		return self.phoneme + self.stress + ' '
