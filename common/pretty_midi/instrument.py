...
class Instrument(object):
    ...
    def __init__(self, program, bank=0, is_drum=False, name='', velocity_offset=0):
        ...
        self.name = name
        self.amplitude_offset = velocity_offset
        self.notes = []
        self.pitch_bends = []
    ...
    def fluidsynth(self, fs=44100, sf2_path=None):
        ...
        # Otherwise just use channel 0
            ...
            channel = 0
            fl.program_select(channel, sfid, self.bank, self.program)
        # Collect all notes in one list
        event_list = []
        for note in self.notes:
            event_list += [[note.start, 'note on', note.pitch, note.velocity]]#note.velocity]]
    ...
    def __repr__(self):
        return 'Instrument(bank = {}, program={}, is_drum={}, name="{}")'.format(
            self.bank, self.program, self.is_drum, self.name.replace('"', r'\"'))
