...
class PrettyMIDI(object):
    ...
    def _load_instruments(self, midi_data):
        ...
        def __get_instrument(bank, program, channel, track, create_new):
            ...
            if create_new:
                ...
                instrument = Instrument(
                    0, program, is_drum, track_name_map[track_idx])
            ...
            # instrument
                ...
                instrument = Instrument(0, program, track_name_map[track_idx])
        ...
        for track_idx, track in enumerate(midi_data.tracks):
            ...
            for event in track:
                    ...
                    if key in last_note_on:
                        ...
                        for start_tick, velocity in notes_to_close:
                            ...
                            instrument = __get_instrument(
                                0, program, event.channel, track_idx, 1)
                            # Add the note event
                ...
                # Store pitch bends
                    ...
                    instrument = __get_instrument(
                        0, program, event.channel, track_idx, 0)
    ...
    def fluidsynth(self, fs=44100, sf2_path=None):
        ...
        if len(self.instruments) == 0 or all(len(i.notes) == 0
            ...
            return np.array([])
        # Get synthesized waveform for each instrument
        waveforms = Parallel(n_jobs=16, prefer="threads")(delayed(i.fluidsynth)(fs, sf2_path) for i in self.instruments)
        ...
        # Sum all waveforms in
        for i, waveform in zip(self.instruments, waveforms):
            if waveform.size == 0:
                continue
                
            synthesized[:waveform.shape[0]] += (waveform / np.max(np.abs(waveform))) * i.amplitude_offset
            total_valid_waveforms += 1

        # Normalize
        synthesized /= total_valid_waveforms
        
        return synthesized.astype(np.float32)

    def write_wav(self, fp, fs=44100, sf2_path=None):
        """Synthesize using fluidsynth and write to wav file
        ...
        synthesized : np.ndarray
            Waveform of the MIDI data, synthesized at ``fs``.
        """
        synthesized = self.fluidsynth(fs, sf2_path)
        scipy.io.wavfile.write(fp, 44100, synthesized)
